...
const providerName = process.env.MODELWEAVE_PROVIDER ?? 'openrouter';
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'modelweave-free-smoke-'));
fs.writeFileSync(path.join(root, 'answer.txt'), 'MODELWEAVE_SMOKE_VALUE=42\n', 'utf8');
...
const provider = createProvider(providerName, { model: process.env.MODELWEAVE_MODEL });
...
const result = await agent.run(
  'Find the exact value of MODELWEAVE_SMOKE_VALUE by using workspace tools. Return only the number.',
