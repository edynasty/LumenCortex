...
- Governor-scheduler validation: opt-in disable gate, revision-delta and cooldown debounce, pressure triggers, deterministic no-Curator path, explicit Curator opt-in, persistent pending plan across scheduler instances, journal entries, dry-run preservation, graph-revision stale-plan rejection, Agent-completion invocation, scheduler-error isolation, and CLI run/status/apply/clear lifecycle.
