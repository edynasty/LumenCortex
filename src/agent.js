...
export class AgentLoop {
  ...
  async run(goal, options = {}) {
    ...
    const context = this.runtime.context(goal, { budgetTokens });
    let session;
    ...
    if (options.sessionId) {
      ...
      session = this.sessions.create({
        ...
        messages,
        metadata: { budgetTokens, maxSteps }
      });
    }

    this.emit('session.start', { sessionId: session.id, goal, selectedNodes: context.selectedNodes.length, contextTokens: context.usedTokens });
    const usage = { promptTokens: 0, completionTokens: 0, totalTokens: 0, requests: 0 };

    for (let step = 1; step <= maxSteps; step += 1) {
      this.emit('llm.request', { sessionId: session.id, step, model: this.provider.model });
      ...
      this.emit('tools.requested', { sessionId: session.id, step, count: calls.length, names: calls.map((call) => call.function?.name) });
      for (const call of calls) {
        ...
        if (parsed.__parse_error) {
          result = { ok: false, content: JSON.stringify({ error: `Invalid tool arguments JSON: ${parsed.__parse_error}`, raw: parsed.__raw }) };
        } else {
          this.emit('tool.start', { sessionId: session.id, step, toolCallId: call.id, name, args: parsed });
          ...
          this.emit('tool.end', { sessionId: session.id, step, toolCallId: call.id, name, ok: result.ok, denied: result.denied, permission: result.permission });
        }

        workspaceMutated ||= Boolean(result.mutatesWorkspace && result.ok);
        stepRecord.toolCalls.push({ id: call.id, name, args: parsed, ok: result.ok, denied: result.denied ?? false });
  ...
  recordTask(session, context, options) {
    ...
    if (options.cognitiveCommit) {
      try { this.repository.commit(`agent: ${session.goal.slice(0, 72)}`, { metadata: { sessionId: session.id, agent: true } }); }
      catch (error) { if (!String(error.message).includes('Nothing to commit')) throw error; }
    }
  }
...
export function formatActiveContext(context) {
  ...
  const edges = context.selectedEdges.slice(0, 256).map((edge) => `${edge.from} -${edge.type}-> ${edge.to}`);
